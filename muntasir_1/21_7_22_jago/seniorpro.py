...
df=df.select('ifa','user.age')
df=df.filter(df.age != -1)
df=df.filter(df.age >= 45)
...
sen=affluence.join(df, on='ifa', how='inner')
...
#joining table
join_df1 = master_df.join(level_df, on='app_level_id', how='inner').cache()
join_df2 = join_df1.join(lifestage_df, on='app_lifestage_id', how='inner').cache()
select_columns = ['bundle', 'app_l1_name', 'app_l2_name', 'app_l3_name','lifestage_name','lifestage_status']
...
app = brq2.join(finalapp_df, on='bundle', how='inner')



app =sen.join(app, on='ifa', how='inner')


app=app.select('ifa','age','final_affluence','lifestage_name')

# df_gender = spark.read.parquet('s3a://ada-prod-data/etl/table/brq/sub/demographics/monthly/BD/202201/gender/*.parquet')


# df_age = spark.read.parquet('s3a://ada-prod-data/etl/table/brq/sub/demographics/monthly/BD/2022{05}/age/*.parquet')
# df_age = df_age.drop('prediction')
# df_age = df_age.withColumnRenamed('label', 'age')


# df_gender = spark.read.parquet('s3a://ada-prod-data/etl/table/brq/sub/demographics/monthly//BD/202205/gender/*.parquet')
