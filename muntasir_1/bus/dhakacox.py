...
for i in range(0,c,1):
  ...
  df=spark.read.parquet(path+'/202204{'+dayone+','+daytwo+'}/*.parquet')
  ...
  df_em=cox_ifa.join(df_em,'ifa','outer')

df_em=df_em.distinct()
df_em.count()

df_em.coalesce(1).write.csv('s3a://ada-bd-emr/muntasir/bus/bus3/cox/april/', mode='overwrite', header=True)

df_em.coalesce(1).write.csv('s3://ada-bd-emr/muntasir/bus/bus3/cox/may/', mode='overwrite', header=True)