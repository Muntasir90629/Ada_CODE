...
s3a://ada-prod-data/etl/data/brq/agg/agg_brq/daily/BD/20210301/*.parquet'
#market share
for m in months:
...
for m in Month2:
    print(m)
    geo = spark.read.parquet('s3a://ada-bd-emr/result/2022/Robi/DSR-623/raw/gp/'+m+'/*.parquet')
    df_gender = spark.read.parquet('s3a://ada-prod-data/etl/table/brq/sub/demographics/monthly/'+country+'/'+m+'/gender/*.parquet')
    df_gender = df_gender.withColumnRenamed('prediction', 'gender')
    df_gender_segment = geo.join(df_gender, ['ifa'], how='left')
    df_gender_segment.groupBy('gender').agg(F.countDistinct('ifa')).orderBy('gender').show(200,0)
...
for i in months:
    print(i)
    geo = spark.read.parquet('s3a://ada-bd-emr/result/2022/Robi/DSR-623/raw/robi/'+m+'/*.parquet')
    df_age = spark.read.parquet('s3a://ada-prod-data/etl/table/brq/sub/demographics/monthly/'+country+'/'+m+'/age/*.parquet')
...
for i in Month2:
    ...
    df_age_segment = geo.join(df_age, ['ifa'], how='left')
    df_age_segment.groupBy('age').agg(F.countDistinct('ifa')).orderBy('age').show(200,0)
geo = spark.read.parquet('s3a://ada-bd-emr/result/2022/Robi/DSR-623/raw/robi/202107/*.parquet')
device = spark.read.parquet('s3a://ada-prod-data/etl/data/brq/sub/device/monthly/'+country+'/202107/*.parquet')
# Affluence Section
for m in months:
    print(m)
    geo = spark.read.parquet('s3a://ada-bd-emr/result/2022/Robi/DSR-623/raw/robi/'+m+'/*.parquet')
    device = spark.read.parquet('s3a://ada-prod-data/etl/data/brq/sub/device/monthly/'+country+'/'+m+'/*.parquet')
...
for m in months:
    print(m)
    brq = spark.read.parquet('s3a://ada-prod-data/etl/data/brq/agg/agg_brq/monthly/'+country+'/'+m+'/*.parquet')
