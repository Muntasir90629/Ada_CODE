...
# tax=tax.withColumn("new_level_id", substring(col("level_id"),1,9))
# tax=tax.drop(tax.level_id)
# tax=tax.withColumnRenamed("new_level_id","level_id")
# tax.select('level_id').show()
...
match_df=match_df.filter(match_df.l1_name=='Financial')




l4=match_df.groupBy("name").agg(F.countDistinct('ifa').alias('count')).sort('count', ascending=False)
