...
geo=spark.read.csv('s3a://ada-bd-emr/muntasir/Stadium/output/fullview-geofence/*.csv',header=True)
geo.printSchema()
...
for day in days:
    ...
    df=spark.read.parquet(path+'/202301{'+day+'}/*.parquet')
    ...
    main_airport=spark.read.csv('s3a://ada-bd-emr/muntasir/airport/ctg_jessore.csv',header=True)
    ...
    dhaka_airport=main_airport.filter(main_airport.name == 'Dhaka airport')
    ...
    cox_airport=main_airport.filter(main_airport.name == 'cox bazaar airport')
    ...
    ctg_airport=main_airport.filter(main_airport.name == 'ctg airport')
...
df_em.coalesce(1).write.csv("ifa3.csv")